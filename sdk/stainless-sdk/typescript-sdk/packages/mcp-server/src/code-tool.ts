...
import { ClientOptions } from 'petstore-sdk-stainless-test';

const prompt = `Runs JavaScript code to interact with the Petstore SDK Stainless Test API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    ...
    PETSTORE_SDK_STAINLESS_TEST_BASE_URL:
      readEnv('PETSTORE_SDK_STAINLESS_TEST_BASE_URL') ?? client.baseURL ?? undefined,
  };
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      project_name: 'petstore-sdk-stainless-test',
      code,
      intent,
      client_opts: {},
...
const localDenoHandler = async ({
  ...
  try {
    const sdkPkgName = 'petstore-sdk-stainless-test';
