...
export const newMcpServer = async ({
  ...
  new McpServer(
    {
      name: 'petstore_sdk_stainless_test_api',
      version: '0.0.1',
    },
    {
...
export async function initMcpServer(params: {
  ...
  let _client: PetstoreSDKStainlessTest | undefined;
  let _clientError: Error | undefined;
  let _logLevel: 'debug' | 'info' | 'warn' | 'error' | 'off' | undefined;

  const getClient = (): PetstoreSDKStainlessTest => {
    if (_clientError) throw _clientError;
    if (!_client) {
      try {
        _client = new PetstoreSDKStainlessTest({
          logger,
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    let client: PetstoreSDKStainlessTest;
    try {
      client = getClient();
    } catch (error) {
      return {
